...
const getDefaultProxyList = () => {
  const now = Date.now()
  if (
    defaultProxyCache.loadedAt &&
    now - defaultProxyCache.loadedAt < DEFAULT_PROXY_CACHE_TTL_MS
  ) {
...
const normalizeProxyConfig = (proxy) => {
  ...
  if (typeof proxy === 'object' && proxy.host && proxy.port) {
    const protocol = proxy.protocol
      ? String(proxy.protocol).replace(':', '').toLowerCase()
      : 'http'
    if (
      ![
        'http',
    ...
    const auth =
      proxy.auth && typeof proxy.auth === 'object'
        ? {
...
const buildProxyUrlFromConfig = (proxyConfig) => {
  ...
  const auth =
    proxyConfig.auth && typeof proxyConfig.auth === 'object'
      ? {
          username: proxyConfig.auth.username
            ? String(proxyConfig.auth.username)
            : '',
          password: proxyConfig.auth.password
            ? String(proxyConfig.auth.password)
            : '',
        }
      : null
...
async function getSocksAgent(proxyUrl, proxyConfigForLog) {
  ...
  try {
    ...
    throw new AccountSyncError(
      'SOCKS5 代理需要安装依赖 socks-proxy-agent（请在 backend 执行 npm i socks-proxy-agent）',
      500
    )
  }
  ...
  if (!SocksProxyAgent) {
    throw new AccountSyncError(
      'SOCKS5 代理依赖 socks-proxy-agent 加载失败',
      500
    )
  }
...
async function requestChatgptText(
  apiUrl,
  { method, headers, data, proxy } = {},
  logContext = {}
) {
  const resolvedProxy = resolveRequestProxy(proxy)
  const rawProxyUrl =
    typeof resolvedProxy === 'string' ? String(resolvedProxy).trim() : ''
  const proxyConfig = normalizeProxyConfig(resolvedProxy)
  const socksProxyUrl =
    proxyConfig && isSocksProxyConfig(proxyConfig)
      ? rawProxyUrl || buildProxyUrlFromConfig(proxyConfig)
      : ''
  const socksAgent = socksProxyUrl
    ? await getSocksAgent(socksProxyUrl, proxyConfig)
    : null
...
export async function fetchOpenAiAccountInfo(token, proxy = null) {
  ...
  const data = parseJsonOrThrow(text, {
    logContext,
    message: 'OpenAI 接口返回格式异常',
  })
  const accountsMap =
    data?.accounts && typeof data.accounts === 'object' ? data.accounts : {}
  const ordering = Array.isArray(data?.account_ordering)
  ...
  const fallbackIds = Object.keys(accountsMap).filter(
    (id) => !orderedIds.includes(id)
  )
  const accountIds = [...orderedIds, ...fallbackIds].filter(
    (id) => id && id !== 'default'
  )
...
const throwChatgptApiStatusError = async ({
  status,
  errorText,
  logContext,
  label,
      ...
      if (code === 'account_deactivated') {
        ...
        if (Number.isFinite(accountId)) {
          try {
            ...
            await saveDatabase()
            console.warn(
              '[AccountSync] upstream account_deactivated; auto-banned',
              { accountId }
            )
          } catch (error) {
        ...
        throw new AccountSyncError(
          'OpenAI 账号已停用（account_deactivated），已自动标记为封号',
          401
        )
      }
    } catch {
...
async function requestAccountInvites(
  account,
  params = {},
  requestOptions = {}
) {
  ...
  const apiUrl = `https://chatgpt.com/backend-api/accounts/${
    account.chatgptAccountId
  }/invites?offset=${offset}&limit=${limit}&query=${encodeURIComponent(query)}`
  const logContext = {
  ...
  const data = parseJsonOrThrow(text, {
    logContext,
    message: 'ChatGPT 邀请响应 JSON 解析失败',
  })
...
async function requestDeleteAccountInvite(
  account,
  emailAddress,
  requestOptions = {}
) {
  const trimmedEmail = String(emailAddress || '')
    .trim()
    .toLowerCase()
  if (!trimmedEmail) {
...
async function requestAccountUsers(account, params = {}, requestOptions = {}) {
  ...
  const apiUrl = `https://chatgpt.com/backend-api/accounts/${
    account.chatgptAccountId
  }/users?offset=${offset}&limit=${limit}&query=${encodeURIComponent(query)}`
  const logContext = {
    accountId: account.id,
    chatgptAccountId: account.chatgptAccountId,
    limit,
  ...
  const data = parseJsonOrThrow(text, {
    logContext,
    message: 'ChatGPT 成员响应 JSON 解析失败',
  })
...
export async function fetchAccountUsersList(accountId, options = {}) {
  const db = await getDatabase()
  const account =
    options.accountRecord || (await fetchAccountById(db, accountId))
  ...
  if (!account.token || !account.chatgptAccountId) {
    throw new AccountSyncError(
      '账号信息不完整，缺少 token 或 chatgpt_account_id',
      400
    )
  }

  const usersData = await requestAccountUsers(account, options.userListParams, {
    proxy: options.proxy,
  })
  return usersData
}
...
export async function fetchAccountInvites(accountId, options = {}) {
  ...
  const invitesData = await requestAccountInvites(
    account,
    options.inviteListParams,
    { proxy: options.proxy }
  )
  return invitesData
}
...
export async function deleteAccountInvite(
  ...
  const result = await requestDeleteAccountInvite(account, trimmedEmail, {
    proxy: options.proxy,
  })
