...
const clampDelay = (value, { min = 0, max = Number.POSITIVE_INFINITY } = {}) =>
...
const isRetryableNetworkError = (error) => {
  ...
  if (message.includes('before secure tls connection was established'))
    return true
...
async function getSocksAgent(proxyUrl) {
  ...
  try {
    ...
    console.error('[ChatGPTInvite] SOCKS 代理依赖缺失（socks-proxy-agent）', {
...
const loadInviteProxyList = () => {
  const overrides = loadProxyList({
    urlsEnvKey: 'CHATGPT_INVITE_PROXY_URLS',
    fileEnvKey: 'CHATGPT_INVITE_PROXY_FILE',
  })
...
const pickProxySequence = (proxies = []) => {
  ...
  return (attemptIndex) =>
    proxies[(start + Math.max(0, attemptIndex - 1)) % proxies.length] || null
}
...
export async function inviteUserToChatGPTTeam(
  email,
  accountData,
  options = {}
) {
  const maxAttempts = Math.max(
    ...
    toInt(
      options.maxAttempts,
      toInt(process.env.CHATGPT_INVITE_RETRY_MAX_ATTEMPTS, DEFAULT_MAX_ATTEMPTS)
    )
  )
  const timeoutMs = Math.max(
    ...
    toInt(
      options.timeoutMs,
      toInt(process.env.CHATGPT_INVITE_TIMEOUT_MS, DEFAULT_TIMEOUT_MS)
    )
  )
  const baseDelayMs = clampDelay(
    toInt(
      options.baseDelayMs,
      toInt(
        process.env.CHATGPT_INVITE_RETRY_BASE_DELAY_MS,
        DEFAULT_RETRY_BASE_DELAY_MS
      )
    ),
    { min: 0 }
  )
  const maxDelayMs = clampDelay(
    toInt(
      options.maxDelayMs,
      toInt(
        process.env.CHATGPT_INVITE_RETRY_MAX_DELAY_MS,
  ...
  const proxyOverrides = options.proxy
    ? [buildProxyConfigFromUrl(options.proxy)].filter(Boolean)
    : []
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (isSocksProxy && proxyUrl) {
      try {
        ...
        lastError = error
        console.error('[ChatGPTInvite] socks agent init failed', {
          proxy: proxyLabel,
          message: error?.message || String(error),
        })
        break
    ...
    try {
      ...
      if (!retryable || attempt >= maxAttempts) {
        return {
          ...
          error: `邀请失败（HTTP ${
            response.status
          }，已尝试 ${attempt} 次）: ${responseText.slice(0, 500)}`,
        }
      }

      const delay = clampDelay(baseDelayMs * 2 ** Math.max(0, attempt - 1), {
        min: 0,
        max: maxDelayMs,
      })
      await sleep(delay)
      ...
      if (!retryable || attempt >= maxAttempts) {
        return {
          success: false,
          error: `邀请失败（已尝试 ${attempt} 次）: ${
            error?.message || String(error) || '邀请接口调用失败'
          }`,
        }
      }
  ...
  return {
    ...
    error: lastError?.message
      ? `邀请失败（已尝试 ${maxAttempts} 次）: ${lastError.message}`
      : '邀请接口调用失败',
  }
}
